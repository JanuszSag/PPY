...
def zapiszDoPliku():
    ...
    for element in dict_list:
        wynik+=element["imie"]+" "+element["nazwisko"]+" "+element["email"]+" "+str(element["punkty"])+" "+str(element["ocena"])+" "+element["status"]+"\n"
        ...
        case "3":
            for element in dict_list:
                if element["status"] != "MAILED":
                    ...
                    sender="" ##należy podać maila
                    recipients=element["email"]
                    password="" ## należy podać hasło do aplikacji
